...
while True:
    ...
    if event == 'Tracking':
        ...
        if values['_SYS_'] is True:
            id_sys = 0
            path_org = Mcr.update_dir(values['_ORI_']) + "\\"
            path_org = r'{}'.format(path_org)
            path_des = Mcr.update_dir(values['_DES_']) + "\\"
    ...
    if eval_c:
        ...
        i += 1
        filenames, image, exp, name = Mcr.load_image_i(path_org, i, type_i, filenames, exp, id_sys)
        ...
        tab_features, feat_track = Mcr.find_seq_feat(i, features_, tab_features, d_max, d_min)
